...
def check_login():
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Accedi"):
            if (
                username in st.secrets["users"]
                and st.secrets["users"][username]["password"] == password
            ):
...
def salva_ticket(
    ...
    citta_cliente, provincia_cliente, contatto, tecnico_id, tecnico_nome,
    citta_tecnico, provincia_tecnico, descrizione, intervento_svolto,
    note, fattura, data_intervento, stato, guasto, allegato_path
):
    """Salva un ticket nel database"""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    ...
    cursor.execute("""
        INSERT INTO ticket (
            ...
            stato, guasto, allegato, data_creazione
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        matricola, modello, cliente_id, codice, azienda, indirizzo_cliente,
        citta_cliente, provincia_cliente, contatto,
...
def pagina_clienti():
    ...
    if df.empty:
        ...
        if not risultati.empty:
            ...
            # Mostriamo più campi nel menu di selezione
