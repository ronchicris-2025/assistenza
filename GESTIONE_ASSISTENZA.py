...
def aggiorna_coordinate_tecnici():
    conn = sqlite3.connect(DB_PATH)
    df = pd.read_sql_query("SELECT id, nome, citta, provincia, regione, lat, lon FROM tecnici", conn)
        ...
        if pd.isnull(row["lat"]) or pd.isnull(row["lon"]):
            ...
            try:
                ...
                if location:
                    ...
                    conn.commit()
                    print(f"✔️ Aggiornato {row['nome']} → {location.latitude}, {location.longitude}")
                else:
                    print(f"⚠️ Non trovato: {location_str}")
            except Exception as e:
                print(f"❌ Errore {row['nome']}: {e}")
            time.sleep(1)
...
def pagina_mappa_tecnici():
    ...
    if st.button("🔄 Aggiorna coordinate tecnici"):
        aggiorna_coordinate_tecnici()
