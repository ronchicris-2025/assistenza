...
# ==============================================
# DATABASE HANDLER INTEGRATO
# ==============================================

def is_streamlit_cloud():
    """Determina se siamo in ambiente Streamlit Cloud"""
    return os.getenv('STREAMLIT_SHARING') or os.getenv('STREAMLIT_RUNTIME')
...
class DatabaseHandler:
    ...
    def connect(self):
        if self.is_cloud:
            # Connessione a PostgreSQL su Supabase
    ...
    def execute(self, query, params=None):
        with self.get_cursor() as cursor:
            # Converti i placeholder da ? a %s per PostgreSQL
            if self.is_cloud:
...
# Istanza globale del database handler
db = DatabaseHandler()

# Funzioni per emulare sqlite3
def connect(database_path=None):
    return SQLiteConnectionEmulator(db)

class SQLiteConnectionEmulator:
    def __init__(self, db_handler):
        self.db_handler = db_handler
    
    def cursor(self):
        return SQLiteCursorEmulator(self.db_handler)
...
class SQLiteCursorEmulator:
    ...
    def execute(self, query, params=None):
        ...
        self._last_row_id = self.db_handler.last_row_id()
    
    def fetchall(self):
        if self._last_query:
            return self.db_handler.fetch_all(self._last_query, self._last_params)
        return []
    
    def fetchone(self):
        if self._last_query:
            return self.db_handler.fetch_one(self._last_query, self._last_params)
        return None
...
def get_assistenza():
    conn = connect()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM assistenza")
    assistenza = cursor.fetchall()
    conn.close()
    return assistenza

def add_assistenza(titolo, descrizione, utente):
    conn = connect()
    cursor = conn.cursor()
    cursor.execute("""
    INSERT INTO assistenza (titolo, descrizione, utente, data_creazione, stato)
    VALUES (?, ?, ?, datetime('now'), 'aperto')
    """, (titolo, descrizione, utente))
    id_assistenza = cursor.lastrowid
    conn.close()
    return id_assistenza

def update_stato_assistenza(id_assistenza, nuovo_stato):
    conn = connect()
    cursor = conn.cursor()
    cursor.execute("UPDATE assistenza SET stato = ? WHERE id = ?", (nuovo_stato, id_assistenza))
    conn.commit()
    conn.close()
...
def importa_codici_excel(db_path="C:/assistenza-app/assistenza.db", excel_name="codici.xlsx"):
    """
    Importa i codici dal file Excel 'codici.xlsx' presente
    nella stessa cartella del database assistenza.db.
    Sovrascrive la tabella 'codici_clienti'.
    """
    # cartella base
    ...
    # leggo l'excel
    df = pd.read_excel(excel_path, dtype={"codice": str})

    # scrivo su DB
    conn = sqlite3.connect(db_path)
    try:
        df.to_sql("codici_clienti", conn, if_exists="replace", index=False)
    finally:
        conn.close()

    print(f"✅ Importati {len(df)} record da {excel_path} in tabella 'codici_clienti'")
...
with st.sidebar:
    ...
    menu = st.radio("Scegli la sezione:", menu_options, index=0)
...
def genera_numero_ticket(azienda):
    from datetime import datetime

    sigla = azienda[:2].upper()
    ...
    cursor.execute("""
        ...
        WHERE substr(numero_ticket, 1, 2) = ?
        AND substr(numero_ticket, 3, 6) = ?
    """, (sigla, oggi))
    count = cursor.fetchone()[0]
    conn.close()

    progressivo = f"{count+1:03d}"
    return f"{sigla}{oggi}{progressivo}"


def salva_ticket(
    numero_ticket, matricola, modello, cliente_id, codice, azienda, indirizzo_cliente,
    citta_cliente, provincia_cliente, regione, contatto,
    ...
    cur.execute("""
        INSERT INTO ticket (
            numero_ticket, matricola, modello, cliente_id, codice, azienda, indirizzo_cliente, citta_cliente, provincia_cliente,
...
def pagina_ticket():
    ...
    clienti_df = pd.read_sql_query("""
        SELECT id, matricola, modello, codice, azienda, indirizzo, citta, provincia, regione, contatto
        FROM clienti
    ...
    # --- Ricerca tecnico ---
    search_tecnico = st.text_input("🔍 Cerca tecnico per nome, città, regione, x provincia (??)")
    if search_tecnico:
    ...
    with st.form("nuovo_ticket_form", clear_on_submit=True):
        ...
        if submitted:
            if not cliente_sel or not tecnico_sel:
                ...
                # Genera numero ticket (azienda viene dal cliente)
                numero_ticket = genera_numero_ticket(cliente["azienda"])
                
                salva_ticket(
                    numero_ticket, matricola, modello, cliente["id"], cliente["codice"], cliente["azienda"],
                    cliente["indirizzo"], cliente["citta"], cliente["provincia"], cliente["regione"],contatto,
                ...
                st.rerun()
    # ===============================
    st.subheader("📋 Visualizza Ticket")
    ...
    if ticket_sel:
        ...
        if ticket_data["allegato"]:
            ...
            try:
                if ext in [".jpg", ".jpeg", ".png"]:
                    st.image(ticket_data["allegato"], caption="Anteprima immagine", use_container_width=True)
                elif ext == ".pdf":
                    from streamlit_pdf_viewer import pdf_viewer
        ...
        with st.form("modifica_ticket_form", clear_on_submit=False):
            ...
            if nuovo_allegato:
                ...
                if ext in ["jpg", "jpeg", "png"]:
                    st.image(nuovo_allegato, caption="Nuova immagine", use_container_width=True)
                elif ext == "pdf":
                    from streamlit_pdf_viewer import pdf_viewer
...
def aggiorna_cliente(row):
    ...
    valori = []
    for col in ["id", "matricola", "matint", "modello", "proprieta","codice", "azienda", "ubicazione", "indirizzo", "citta", "provincia", "contatto", "vestizione", "note", "ddt", "regione"]:
        val = row[col]
    ...
    cur.execute("""
        INSERT INTO clienti (id, matricola, matint, modello, proprieta, codice, azienda, ubicazione, indirizzo, citta, provincia, contatto, vestizione, note, ddt, regione)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)
...
def reset_ticket():
    ...
    conn.commit()
    conn.close()
def aggiorna_regioni_clienti(db_path=DB_PATH):
    """Aggiorna la colonna regione in clienti in base alla tabella province"""
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()

    # assicurati che esista la colonna regione nella tabella clienti
    cur.execute("PRAGMA table_info(clienti)")
    colonne = [c[1] for c in cur.fetchall()]
    if "regione" not in colonne:
        cur.execute("ALTER TABLE clienti ADD COLUMN regione TEXT")

    # aggiorna usando join
    cur.execute("""
        UPDATE clienti
        SET regione = (
            SELECT p.regione FROM province p WHERE p.provincia = clienti.provincia
        )
    ...
    conn.commit()
    conn.close()
    print("✅ Clienti aggiornati con la regione")
...
# -------------------------
    ...
    if not df_clienti.empty:
        ...
        if province_scoperte:
            ...
            # 👇 controllo warning a livello provincia
                clienti_con_ticket = clienti_prov["matricola"].isin(df_ticket["matricola"]).any()
                warning = "⚠️" if clienti_con_ticket else ""
    ...
    # 👉 Da inserire alla fine di pagina_clienti()
    st.markdown("---")
    st.subheader("🔄 Aggiorna Regione nei Clienti")

    if st.button("📌 Aggiorna regioni dai dati province"):
        try:
            aggiorna_regioni_clienti()
            st.success("✅ Regioni aggiornate correttamente nei clienti.")
        except Exception as e:
            st.error(f"❌ Errore durante aggiornamento regioni: {e}")    
...
# -------------------------
    ...
    # Carichiamo tabella codici_clienti
    df_codici = pd.read_sql_query("SELECT * FROM codici_clienti", conn)
    conn.close()

    # Ricerca codice per indirizzo o città
    search_codice = st.text_input("🔍 CERCA PUNTO VENDITA ( x indirizzo o città)")
    codice_sel = None
    precompilati = {}

    if search_codice and len(search_codice) >= 3:
        risultati = df_codici[
            df_codici["indirizzo"].str.contains(search_codice, case=False, na=False) |
            df_codici["citta"].str.contains(search_codice, case=False, na=False)
        ]

        if not risultati.empty:
            risultati["label"] = risultati.apply(
                lambda x: f"{x.get('codice','')} | {x.get('indirizzo','')} | {x.get('citta','')} "
                f"({x.get('provincia','')}) | {x.get('insegna','')} | {x.get('sub_insegna','')} "
                f"| {x.get('gruppo_commerciale','')}",
                axis=1
            )
            codice_sel = st.selectbox("Seleziona punto vendita trovato", risultati["label"])

            if codice_sel:
                riga = risultati[risultati["label"] == codice_sel].iloc[0]
                precompilati = {
                    "codice": riga["codice"],
    ...
    with st.form("aggiungi_cliente"):
        ...
        for col in colonne_clienti:
            if col in precompilati:
                nuovi_valori[col] = st.text_input(col, value=precompilati[col])
            else:
        ...
        if submitted:
            if any(nuovi_valori.values()):
                matricola = nuovi_valori.get("matricola", "").strip()

                try:
                    conn = sqlite3.connect(DB_PATH)
                    cur = conn.cursor()

                # Se matricola è valorizzata → deve essere unica
                    if matricola:
                        cur.execute("SELECT 1 FROM clienti WHERE matricola = ?", (matricola,))
                        if cur.fetchone():
                            st.error("❌ Errore: la matricola inserita esiste già.")
                            conn.close()
                            st.stop()
    ...
    # 🔎 Input ricerca libero (può essere matricola, azienda, indirizzo o codice)
    search_term = st.text_input("🔍 Cerca cliente (matricola, azienda, codice, proprieta o indirizzo)")

    if search_term:
        # Filtriamo su più colonne
        risultati = df[
            df.apply(
                lambda x: search_term.lower() in str(x["matricola"]).lower()
                or search_term.lower() in str(x["proprieta"]).lower()
                or search_term.lower() in str(x["codice"]).lower()
                or search_term.lower() in str(x["azienda"]).lower()
                or search_term.lower() in str(x["indirizzo"]).lower(),
                axis=1
            )
        ]
    else:
    ...
    if not risultati.empty:
        ...
        risultati["label"] = risultati.apply(
            lambda x: f"[ID:{x['id']}] Proprieta:{x['proprieta']} | Codice:{x['codice']} | {x['azienda']} | {x['indirizzo']} | Matricola:{x.get('matricola', '')}",
            axis=1
        )
        ...
        if st.button("⚠️ Conferma eliminazione"):
            ...
            try:
                ...
                st.error(
                    f"❌ Impossibile eliminare il cliente: {e}. "
                    "Verifica se esistono record collegati (es. ticket) "
                    "o aggiungi ON DELETE CASCADE al vincolo."
                )
    else:
        if search_term:
            st.warning("⚠️ Nessun cliente trovato con i criteri di ricerca.")
    ...
    # -------------------------
    # RESET COMPLETO CLIENTI
    # -------------------------
    st.subheader("🧹 Reset Database Clienti")

    def apri_dialog_reset_clienti():
        @st.dialog("Conferma reset clienti", width="small")
        def _dlg():
            st.warning("❓ Sei sicuro di voler cancellare TUTTI i clienti? Operazione irreversibile.")
            c1, c2 = st.columns(2)
            with c1:
                if st.button("✅ Conferma", key="btn_confirm_reset_dialog"):
                    reset_clienti()
                    st.success("✅ Tutti i clienti eliminati e ID resettato!")
                    st.session_state["dialog_open"] = False
                    st.rerun()
            with c2:
                if st.button("❌ Annulla", key="btn_cancel_reset_dialog"):
                    st.session_state["dialog_open"] = False
                    st.rerun()
        _dlg()

    # inizializzo lo stato se non esiste
    if "dialog_open" not in st.session_state:
        st.session_state["dialog_open"] = False

    # apertura del dialog
    if st.button("⚠️ Elimina tutti i clienti e resetta ID", key="btn_open_reset_dialog"):
        st.session_state["dialog_open"] = True
        st.rerun()

    # mostro il dialog solo se lo stato è attivo
    if st.session_state["dialog_open"]:
        apri_dialog_reset_clienti()
...
def pagina_tecnici():
    ...
    if df.empty:
        ...
        # Confrontiamo i dati originali con quelli modificati
        ...
        # Aggiungi un messaggio per aggiornare le coordinate
        st.info("🔄 Per aggiornare le coordinate dei tecnici, vai alla pagina 'Mappa Tecnici' e clicca su 'Aggiorna coordinate tecnici'")
...
def pagina_import_export():
        ...
        if st.button("📥 Esporta Ticket (Excel)"):
            ...
            df = pd.read_sql_query("""
                SELECT
                    ...
                    t.stato,
                    t.data_creazione,
                -- campi storici già dentro ticket
                    t.matricola,
                    t.azienda,
                    t.indirizzo_cliente,
                    t.citta_cliente,
    ...
    # --- Upload Ticket (Excel) con colmap/freeze_cols in-scope e usati ---
            ...
            if not st.session_state.upload_done:
                if st.button("✅ Conferma upload ticket"):
                    ...
                    for _, r in df.iterrows():
                        ...
                        if not matricola or not descrizione:
                            skip += 1
                            continue

                        # Campi opzionali dal file Excel (se non ci sono, default vuoto/0)
                        ...
                        numero_ticket = str(r.get("numero_ticket") or "")
                        cliente_id= str(r.get("cliente_id") or "")

                        # Campi cliente presi DIRETTAMENTE dal file Excel
                        azienda = str(r.get("azienda") or "")
                        indirizzo_cliente = str(r.get("indirizzo_cliente") or "")
                        citta_cliente = str(r.get("citta_cliente") or "")
                        provincia_cliente = str(r.get("provincia_cliente") or "")
                        regione = str(r.get("regione") or "")
                        contatto = str(r.get("contatto") or "")
                        modello = str(r.get("modello") or "")

                        # Data creazione: se presente nel file la usiamo, altrimenti ora
                        data_creazione = to_safe_date_str(r.get("data_creazione")) or datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        ...
                        # Inserimento ticket con i dati presi dal file
                        cur.execute("""
                            INSERT INTO ticket (
                                matricola, modello, cliente_id, descrizione, fattura, note, data_intervento,
                                intervento_svolto, stato, allegato, contatto, regione, guasto,
                                tecnico_nome, citta_tecnico, provincia_tecnico, numero_ticket,
                                azienda, indirizzo_cliente, citta_cliente, provincia_cliente,
                                data_creazione
                            ) VALUES ( ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            matricola, modello, cliente_id, descrizione, fattura, note, data_interv,
                            intervento, stato, allegato, contatto, regione, guasto,
                            tecnico_nome, citta_tecnico, provincia_tecnico, numero_ticket,
                            azienda, indirizzo_cliente, citta_cliente, provincia_cliente,
                            data_creazione
                        ))
    ...
    # --- Upload file CODICI_UNIVOCI (Excel) con controllo ed inserimento zeri su campo "codice" ---

    st.subheader("📥 Importa codici punti vendita")

    uploaded_file = st.file_uploader("Carica file Excel codici", type=["xlsx"])

    if uploaded_file is not None:
        try:
            # Legge l'Excel come stringhe
            df = pd.read_excel(uploaded_file, dtype=str)

            # Normalizza la colonna codice
            if "codice" in df.columns:
                df["codice"] = df["codice"].astype(str).str.strip()
                df["codice"] = df["codice"].apply(
                    lambda x: x.zfill(8) if x.isdigit() else x
                )
            ...
            st.success(f"✅ Importati {len(df)} codici in tabella 'codici_clienti'")
 ...
 # --- Upload file PROVINCE " ---

    st.subheader("📥 Importa file Province/regioni")

    uploaded_file = st.file_uploader("Carica file province", type=["xlsx"])
    ...
    if uploaded_file is not None:
        try:
            ...
            # Scrive nel DB
            conn = sqlite3.connect(DB_PATH)
            df.to_sql("province", conn, if_exists="replace", index=False)
            conn.close()

            st.success(f"✅ Importati {len(df)} codici in tabella 'Province'")
...
# VERIFICA PERSISTENZA DATI INSERITI DURANTE SESSIONE (STREAMLIT CLOUD)
            
# Funzione per ottenere il percorso del database
def get_database_path():
    ...
    if os.environ.get('STREAMLIT_CLOUD') == 'true':
        ...
        return os.path.join('.streamlit', 'assistenza.db')
    else:
        # In locale
        return os.path.join(os.getcwd(), 'assistenza.db')
...
def init_database():
    ...
    try:
        # Esempio di tabella per le attività
        cursor.execute('''
...
# Funzione per recuperare tutte le tabelle utente nel database (MODIFICATA)
def get_user_tables():
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        # Filtra le tabelle di sistema SQLite
        system_tables = ['sqlite_sequence', 'sqlite_stat1', 'sqlite_stat2', 'sqlite_stat3', 'sqlite_stat4']
        user_tables = [table for table in all_tables if not table.startswith('sqlite_') and table not in system_tables]
...
# Funzione per ottenere gli ultimi N record di una tabella (MODIFICATA)
def get_last_n_records(table_name, n=5):
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        # Trova la colonna chiave primaria o usa 'rowid' come fallback
        primary_key = None
        for col in columns_info:
            if col[5] == 1:  # 5° elemento è l'indicatore di chiave primaria
                primary_key = col[1]
                break
        
        # Se non c'è una chiave primaria, usa 'rowid'
        order_by = primary_key if primary_key else 'rowid'
        
        # Query per ottenere gli ultimi N record
        cursor.execute(f"SELECT * FROM {table_name} ORDER BY {order_by} DESC LIMIT {n}")
        ...
        st.error(f"Errore nel recupero degli ultimi record per la tabella {table_name}: {e}")
        return pd.DataFrame()

# Funzione principale per la verifica di tutte le tabelle (MODIFICATA)
def verify_all_tables(n_records=5):
    # Inizializza il database se necessario
    init_database()
    
    # Ottieni solo le tabelle utente
    tables = get_user_tables()
    
    if not tables:
        st.warning("Nessuna tabella utente trovata nel database. Il database è vuoto o non inizializzato.")
        st.info("Inserisci alcuni dati nell'app per creare le tabelle.")
        return
    
    st.write(f"### Verifica degli ultimi {n_records} record per ogni tabella")
    
    for table in tables:
        st.write(f"#### Tabella: {table}")
        df = get_last_n_records(table, n_records)
        ...
        if not df.empty:
            ...
            with col1:
                st.write(f"**Numero di record:** {len(df)}")
            with col2:
                st.write(f"**Colonne:** {', '.join(df.columns)}")
        else:
            st.write("Nessun record trovato in questa tabella")
        
        st.markdown("---")  # Separatore tra le tabelle
...
def pagina_tools():
    ...
    st.header("Verifica Dati nel Database")
    
    # Informazioni sul database
    db_path = get_database_path()
    if os.path.exists(db_path):
        st.success(f"Database trovato: {db_path}")
        st.write(f"Dimensione del file: {os.path.getsize(db_path) / 1024:.2f} KB")
    else:
        st.warning("Database non trovato. Verrà creato al primo inserimento dati.")
    
    # Slider per scegliere il numero di record da visualizzare
    n_records = st.slider("Numero di record da visualizzare per tabella", min_value=1, max_value=20, value=5)
    
    # Pulsante per avviare la verifica
    if st.button("Verifica Ultimi Record"):
        with st.spinner("Recupero dati in corso..."):
            verify_all_tables(n_records)
    
    # Sezione per gestione database
    #st.subheader("Gestione Database")
    
    #if st.button("Reset Database"):
        #if os.path.exists(db_path):
            #os.remove(db_path)
            #st.success("Database eliminato! Verrà ricreato al prossimo avvio.")
            #st.experimental_rerun()
        #else:
            #st.info("Il database non esiste.")
    
    # Mostra informazioni aggiuntive sul database
    with st.expander("Informazioni sul database"):
        conn = get_db_connection()
        cursor = conn.cursor()
        
        try:
            # Mostra tutte le tabelle (incluse quelle di sistema)
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
            all_tables = [table[0] for table in cursor.fetchall()]
            
            st.write("**Tutte le tabelle nel database:**")
            st.json(all_tables)
            
            # Mostra le tabelle utente
            user_tables = get_user_tables()
            st.write("**Tabelle utente:**")
            st.json(user_tables)
            
            # Mostra le tabelle di sistema
            system_tables = [table for table in all_tables if table not in user_tables]
            st.write("**Tabelle di sistema:**")
            st.json(system_tables)
            
        except Exception as e:
            st.error(f"Errore nel recupero delle informazioni: {e}")
...
def genera_pdf_ticket_fattura(ticket_data, campi):
    ...
    # Logo opzionale
    if "Logo" in campi and os.path.exists(LOGO_PATH):
        logo = Image(LOGO_PATH, width=3.4*cm, height=2.2*cm)
        logo.hAlign = 'LEFT'   # puoi cambiare in 'RIGHT' o 'CENTER'
        elementi.append(logo)
        elementi.append(Spacer(10, 10))

    # Titolo documento
    titolo = Paragraph(
        f"<para align=left><b>Ticket | {safe_str(ticket_data.get('numero_ticket'))}</b></para>",
        styleH
    )
    elementi.append(titolo)
    titolo = Paragraph(
        f"<para align=left><b>Date|time creazione : {safe_str(ticket_data.get('data_creazione'))}</b></para>",
        styleN
    )
    elementi.append(titolo)
    elementi.append(Spacer(10, 10))
...
def pagina_stampa_ticket():
    ...
    query = """
        SELECT t.*
        FROM ticket t
        WHERE t.matricola LIKE ? 
           OR t.azienda LIKE ? 
           OR t.tecnico_nome LIKE ? 
           OR t.descrizione LIKE ? 
           OR t.guasto LIKE ?
    ...
    # convertiamo subito i valori
    tickets = []
    for row in rows:
        rec = {}
        for col, val in zip(cols, row):
            if col == "fattura":  # mantieni numerico
    ...
    if ticket_sel:
        ...
        campi_possibili = [
            "Logo", "Tecnico Frigorista", "Criticità segnalata",
        ...
        if st.button("📄 Genera PDF"):
            ...
            pdf_viewer(pdf_bytes, width=780)

            # download
            st.download_button(
                label="⬇️ Scarica PDF",
                data=pdf_bytes,
                file_name=f"ticket_{ticket_data['numero_ticket']}.pdf",
...
# Funzione per geocodificare città/provincia
def geocodifica(citta, provincia=None):
    geolocator = Nominatim(user_agent="assistenza_app")
    geocode = RateLimiter(geolocator.geocode, min_delay_seconds=1)
    if provincia:
        query = f"{citta}, {provincia}, Italia"
    else:
...
def aggiorna_coordinate_tecnici():
    print("DEBUG: Inizio aggiornamento coordinate tecnici")
    conn = sqlite3.connect(DB_PATH)
    
    try:
        # Verifica prima quanti tecnici hanno bisogno di aggiornamento
        cursor = conn.cursor()
        # Modifica la query per includere anche valori vuoti o non validi
        cursor.execute("SELECT COUNT(*) FROM tecnici WHERE lat IS NULL OR lon IS NULL OR lat = '' OR lon = '' OR lat = '0' OR lon = '0'")
        da_aggiornare = cursor.fetchone()[0]
        print(f"DEBUG: Tecnici da aggiornare: {da_aggiornare}")
        
        if da_aggiornare == 0:
            print("DEBUG: Nessun tecnico da aggiornare")
            return ["ℹ️ Tutti i tecnici hanno già le coordinate"]
        
        # Modifica anche questa query per includere valori vuoti o non validi
        df = pd.read_sql_query("SELECT id, nome, citta, provincia, regione, lat, lon FROM tecnici WHERE lat IS NULL OR lon IS NULL OR lat = '' OR lon = '' OR lat = '0' OR lon = '0'", conn)
        print(f"DEBUG: Caricati {len(df)} tecnici da aggiornare")
        
        geolocator = Nominatim(user_agent="gestione_assistenza")
        geocode = RateLimiter(geolocator.geocode, min_delay_seconds=1)
        
        risultati = []
        
        for index, row in df.iterrows():
            print(f"DEBUG: Elaborazione tecnico {index+1}/{len(df)}: {row['nome']} (ID: {row['id']})")
            print(f"DEBUG: Coordinate attuali: lat={row['lat']}, lon={row['lon']}")
            
            # Salta tecnici senza città o provincia
            if not row['citta'] or not row['provincia'] or pd.isna(row['citta']) or pd.isna(row['provincia']):
                msg = f"⚠️ Saltato {row['nome']}: città o provincia mancanti"
                print(f"DEBUG: {msg}")
                risultati.append(msg)
                continue
                
            location_str = f"{row['citta']}, {row['provincia']}, Italia"
            print(f"DEBUG: Geocodifica di '{location_str}'")
            ...
            try:
                ...
                if location:
                    ...
                    print(f"DEBUG: Coordinate trovate: {lat}, {lon}")
                    
                    # Aggiorna il database
                    cursor.execute(
                        "UPDATE tecnici SET lat=?, lon=? WHERE id=?",
                        (lat, lon, row["id"])
                    )
                    conn.commit()
                    
                    # Verifica che l'aggiornamento sia stato eseguito
                    cursor.execute("SELECT lat, lon FROM tecnici WHERE id=?", (row["id"],))
                    result = cursor.fetchone()
                    
                    if result and result[0] is not None and result[1] is not None:
                        msg = f"✔️ Aggiornato {row['nome']} → {result[0]}, {result[1]}"
                        print(f"DEBUG: {msg}")
                        risultati.append(msg)
                    else:
                        msg = f"❌ Errore verifica aggiornamento per {row['nome']}"
                        print(f"DEBUG: {msg}")
                        risultati.append(msg)
                else:
                    msg = f"⚠️ Non trovato: {location_str}"
                    print(f"DEBUG: {msg}")
                    risultati.append(msg)
            except Exception as e:
                msg = f"❌ Errore {row['nome']}: {str(e)}"
                print(f"DEBUG: {msg}")
                risultati.append(msg)
            
            # Pausa per non sovraccaricare il servizio di geocodifica
            time.sleep(1)
        
        print("DEBUG: Fine aggiornamento coordinate tecnici")
        return risultati
...
# Funzione per verificare lo stato delle coordinate
def verifica_coordinate_tecnici():
    conn = sqlite3.connect(DB_PATH)
    
    try:
        # Ottieni tutti i tecnici
        df = pd.read_sql_query("SELECT id, nome, citta, provincia, lat, lon FROM tecnici", conn)
        
        # Conta i tecnici senza coordinate (inclusi valori vuoti o non validi)
        senza_coordinate = df[
            pd.isnull(df['lat']) | 
            pd.isnull(df['lon']) | 
            (df['lat'] == '') | 
        ...
        con_coordinate = df.dropna(subset=['lat', 'lon'])
        con_coordinate = con_coordinate[
            (con_coordinate['lat'] != '') & 
            (con_coordinate['lon'] != '') &
            (con_coordinate['lat'] != '0') &
            (con_coordinate['lon'] != '0')
        ]
        
        st.write(f"**Totale tecnici:** {len(df)}")
        st.write(f"**Tecnici con coordinate:** {len(con_coordinate)}")
        st.write(f"**Tecnici senza coordinate:** {len(senza_coordinate)}")
        
        if not senza_coordinate.empty:
            st.write("**Tecnici senza coordinate:**")
            st.dataframe(senza_coordinate[['id', 'nome', 'citta', 'provincia', 'lat', 'lon']])
            
        if not con_coordinate.empty:
            st.write("**Esempio di tecnici con coordinate:**")
            st.dataframe(con_coordinate[['id', 'nome', 'citta', 'provincia', 'lat', 'lon']].head(5))
...
def mostra_mappa_tecnici(regioni_sel, citta_sel=None, raggio_km=None, num_punti=None):
    conn = sqlite3.connect(DB_PATH)
    df_tecnici = pd.read_sql_query(
        "SELECT id, esperienza, nome, citta, provincia, regione, referente, telefono, cellulare, email, lat, lon FROM tecnici", conn
    )
    conn.close()

    if df_tecnici.empty:
        st.warning("⚠️ Nessun tecnico presente in archivio.")
        return

    # Filtra per regioni
    if "Tutte" in regioni_sel or len(regioni_sel) == 0:
        df_filtrato = df_tecnici
    else:
        df_filtrato = df_tecnici[df_tecnici["regione"].isin(regioni_sel)]

    if df_filtrato.empty:
        st.warning("⚠️ Nessun tecnico trovato per le regioni selezionate.")
        return

    # Pulisci i dati delle coordinate
    # Converti stringhe vuote in NaN
    df_filtrato['lat'] = pd.to_numeric(df_filtrato['lat'], errors='coerce')
    df_filtrato['lon'] = pd.to_numeric(df_filtrato['lon'], errors='coerce')
    
    # Rimuovi tecnici senza coordinate valide
    df_filtrato = df_filtrato.dropna(subset=["lat", "lon"])

    # Se è stata selezionata una città, calcola le distanze e filtra
    if citta_sel:
        # Geocodifica la città selezionata
        lat_citta, lon_citta = geocodifica(citta_sel)
        if lat_citta is None or lon_citta is None:
            st.error(f"⚠️ Impossibile trovare le coordinate per la città: {citta_sel}")
            st.info("💡 Suggerimento: Prova a specificare anche la provincia (es: 'Roma, RM')")
            return

        # Calcola la distanza per ogni tecnico
        distanze = []
        for _, row in df_filtrato.iterrows():
            distanza = calcola_distanza(lat_citta, lon_citta, row["lat"], row["lon"])
            distanze.append(distanza)

        df_filtrato["distanza_km"] = distanze

        # Rimuovi i tecnici senza distanza valida
        df_filtrato = df_filtrato.dropna(subset=["distanza_km"])

        # Ordina per distanza
        df_filtrato = df_filtrato.sort_values(by="distanza_km")

        # Filtra per raggio o per numero di punti
        if raggio_km is not None:
            df_filtrato = df_filtrato[df_filtrato["distanza_km"] <= raggio_km]
        elif num_punti is not None:
            df_filtrato = df_filtrato.head(num_punti)

        # Centra la mappa sulla città selezionata
        centro_mappa = [lat_citta, lon_citta]
        zoom_start = 10
    else:
        # Mappa centrata sull'Italia
        centro_mappa = [41.8719, 12.5674]
        zoom_start = 6

    # Controlla se ci sono tecnici da mostrare
    if df_filtrato.empty:
        st.warning("⚠️ Nessun tecnico trovato con i criteri selezionati.")
        return

    # Crea la mappa
    mappa = folium.Map(location=centro_mappa, zoom_start=zoom_start)

    # Aggiungi un marker per la città selezionata (se presente)
    if citta_sel and lat_citta and lon_citta:
        folium.Marker(
            location=[lat_citta, lon_citta],
            popup=f"📍 {citta_sel}",
            icon=folium.Icon(color="red", icon="home", prefix="fa")
        ).add_to(mappa)
     # Mappatura esperienza -> (icona + descrizione)
    esperienza_info = {
        'OK': {'icona': '👍', 'descrizione': 'Tecnico OK'},
        'TESTARE': {'icona': '⚖️', 'descrizione': 'Nuovo tecnico da provare'},
        'NOK': {'icona': '👎', 'descrizione': 'Tecnico NON OK'}
    }
     # Aggiungi i marker dei tecnici
    ### for _, row in df_tecnici.iterrows():
    for _, row in df_filtrato.iterrows():
        # Ottieni informazioni sull'esperienza
        esp_info = esperienza_info.get(row['esperienza'], {})
        icona = esp_info.get('icona', '')
        descrizione = esp_info.get('descrizione', '')# Aggiungi i marker dei tecnici
    
        # Se è stata selezionata una città, mostra la distanza nel popup
        if citta_sel and "distanza_km" in row:
            popup = f"""
            <div style="width: 300px;">
            📏 <b>Distanza:</b> {row['distanza_km']:.2f} km<br>
            ...
            <hr style="margin: 1px 0;">
            👤 <b>Referente:</b> {row['referente']}<br>
            <hr style="margin: 1px 0;">
            📞 <b>Telefono:</b> {row['telefono']}<br>
            <hr style="margin: 1px 0;">
            📱 <b>Cellulare:</b> {row['cellulare']}<br>
            <hr style="margin: 1px 0;">
            @ <b>e_mail:</b> {row['email']}<br>
        </div>
        """
        else:
            popup = f"""
        ...
        """
       # Creazione del marker con popup personalizzato
    ...
    st_folium(mappa, width=800, height=550)

# --- Pagina Streamlit ---
def pagina_mappa_tecnici():
    st.header("🌍👷 Mappa Tecnici")

    # Verifica lo stato delle coordinate
    with st.expander("📊 Stato delle coordinate"):
        verifica_coordinate_tecnici()

    # Aggiungi un'opzione per aggiornare tutte le coordinate
    with st.expander("🛠️ Opzioni avanzate"):
        st.write("Aggiorna le coordinate di tutti i tecnici senza coordinate (potrebbe richiedere tempo)")
        if st.button("🔄 Aggiorna coordinate di tutti i tecnici"):
            with st.spinner("Aggiornamento in corso..."):
                risultati = aggiorna_coordinate_tecnici()
                st.success("Aggiornamento completato!")
                st.subheader("Risultati dell'aggiornamento:")
                for risultato in risultati:
                    st.write(risultato)
                
                # Aggiungi un pulsante per ricaricare la pagina
                if st.button("🔄 Ricarica pagina per vedere le modifiche"):
                    st.rerun()


    conn = sqlite3.connect(DB_PATH)
    regioni = pd.read_sql_query("SELECT DISTINCT regione FROM tecnici WHERE regione IS NOT NULL", conn)
    citta_disponibili = pd.read_sql_query("SELECT DISTINCT citta FROM tecnici WHERE citta IS NOT NULL", conn)
    conn.close()

    lista_regioni = ["Tutte"] + sorted(regioni["regione"].dropna().tolist())
    regioni_sel = st.multiselect("📌 Seleziona regioni da visualizzare:", lista_regioni, default=["Tutte"])

    # Input per la città con suggerimenti
    st.subheader("🔍 Cerca una città")
    
    # Opzione: usare città suggerite o inserirne una nuova
    use_suggested = st.checkbox("Usa città dove sono presenti i tecnici", value=False)
    
    if use_suggested:
        citta_sel = st.selectbox(
            "Seleziona una città:", 
            options=[""] + sorted(citta_disponibili["citta"].dropna().tolist()),
            index=0
        )
    else:
        citta_sel = st.text_input(
            "Inserisci il nome della città (opzionale: aggiungi provincia, es: 'Roma, RM'):", 
            ""
        )

    # Opzioni di ricerca: per raggio o per numero di punti
    opzione_ricerca = st.radio("Opzione di ricerca:", ["Raggio (km)", "Numero di punti più vicini"], index=0)
    if opzione_ricerca == "Raggio (km)":
        raggio_km = st.number_input("Raggio di ricerca (km):", min_value=1, value=50)
        num_punti = None
    else:
        num_punti = st.number_input("Numero di punti da mostrare:", min_value=1, value=5)
        raggio_km = None

    # Se l'utente ha inserito una città, usiamola per la ricerca
    if citta_sel:
        mostra_mappa_tecnici(regioni_sel, citta_sel, raggio_km, num_punti)
    else:
        mostra_mappa_tecnici(regioni_sel)
...
# Funzione per verificare lo stato delle coordinate dei clienti
def verifica_coordinate_clienti():
    conn = sqlite3.connect(DB_PATH)
    
    try:
        # Ottieni tutti i clienti
        df = pd.read_sql_query("SELECT id, azienda, citta, provincia, codice, matricola, lat, lon, proprieta FROM clienti", conn)
        
        # Conta i clienti senza coordinate (inclusi valori vuoti o non validi)
        senza_coordinate = df[
            pd.isnull(df['lat']) | 
            pd.isnull(df['lon']) | 
            (df['lat'] == '') | 
        ...
        st.write(f"**Totale clienti:** {len(df)}")
        st.write(f"**Clienti con coordinate:** {len(con_coordinate)}")
        st.write(f"**Clienti senza coordinate:** {len(senza_coordinate)}")
        
        if not senza_coordinate.empty:
            st.write("**Clienti senza coordinate:**")
            st.dataframe(senza_coordinate[['id', 'azienda', 'citta', 'provincia', 'lat', 'lon']])
            
        if not con_coordinate.empty:
            st.write("**Esempio di clienti con coordinate:**")
            st.dataframe(con_coordinate[['id', 'azienda', 'citta', 'provincia', 'lat', 'lon']].head(5))
...
# Funzione per aggiornare le coordinate dei clienti
def aggiorna_coordinate_clienti():
    print("DEBUG: Inizio aggiornamento coordinate clienti")
    conn = sqlite3.connect(DB_PATH)
    
    try:
        # Verifica prima quanti clienti hanno bisogno di aggiornamento
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM clienti WHERE lat IS NULL OR lon IS NULL OR lat = '' OR lon = '' OR lat = '0' OR lon = '0'")
        da_aggiornare = cursor.fetchone()[0]
        print(f"DEBUG: Clienti da aggiornare: {da_aggiornare}")
        
        if da_aggiornare == 0:
            print("DEBUG: Nessun cliente da aggiornare")
            return ["ℹ️ Tutti i clienti hanno già le coordinate"]
        
        df = pd.read_sql_query("SELECT id, azienda, citta, provincia, codice, matricola, lat, lon, proprieta FROM clienti WHERE lat IS NULL OR lon IS NULL OR lat = '' OR lon = '' OR lat = '0' OR lon = '0'", conn)
        print(f"DEBUG: Caricati {len(df)} clienti da aggiornare")
        ...
        for index, row in df.iterrows():
            print(f"DEBUG: Elaborazione cliente {index+1}/{len(df)}: {row['azienda']} (ID: {row['id']})")
            
            if not row['citta'] or not row['provincia'] or pd.isna(row['citta']) or pd.isna(row['provincia']):
                msg = f"⚠️ Saltato {row['azienda']}: città o provincia mancanti"
                print(f"DEBUG: {msg}")
                risultati.append(msg)
                continue
            ...
            try:
                ...
                if location:
                    ...
                    cursor.execute(
                        "UPDATE clienti SET lat=?, lon=? WHERE id=?",
                        (lat, lon, row["id"])
                    )
                    conn.commit()
                    
                    cursor.execute("SELECT lat, lon FROM clienti WHERE id=?", (row["id"],))
                    result = cursor.fetchone()
                    
                    if result and result[0] is not None and result[1] is not None:
                        msg = f"✔️ Aggiornato {row['azienda']} → {result[0]}, {result[1]}"
                        print(f"DEBUG: {msg}")
                        risultati.append(msg)
                    else:
                        msg = f"❌ Errore verifica aggiornamento per {row['azienda']}"
                        print(f"DEBUG: {msg}")
                        risultati.append(msg)
                else:
                    msg = f"⚠️ Non trovato: {location_str}"
                ...
                msg = f"❌ Errore {row['azienda']}: {str(e)}"
                print(f"DEBUG: {msg}")
                risultati.append(msg)
            
            time.sleep(1)
        
        print("DEBUG: Fine aggiornamento coordinate clienti")
        return risultati
...
# --- Funzione: mostra mappa clienti ---
def mostra_mappa_clienti(regioni_sel, citta_sel=None, raggio_km=None, num_punti=None, proprieta_sel=None):
    conn = sqlite3.connect(DB_PATH)
    df_clienti = pd.read_sql_query(
        "SELECT id, azienda, citta, indirizzo, provincia, codice, matricola, matint, lat, lon, proprieta FROM clienti", conn
    )
    conn.close()

    if df_clienti.empty:
        st.warning("⚠️ Nessun cliente presente in archivio.")
        return

    # Filtra per proprietà
    if proprieta_sel and proprieta_sel != "Tutti":
        df_clienti = df_clienti[df_clienti["proprieta"] == proprieta_sel]

    # Filtra per regioni
    if "Tutte" in regioni_sel or len(regioni_sel) == 0:
        df_filtrato = df_clienti.copy()
    else:
        # Dobbiamo prima estrarre la regione dai clienti
        df_filtrato = df_clienti.copy()
        
        # Se non c'è un campo regione, lo aggiungiamo in base alla provincia
        if 'regione' not in df_filtrato.columns:
            # Mappatura province -> regioni (semplificata)
            regioni_map = {
                'RM': 'Lazio', 'MI': 'Lombardia', 'NA': 'Campania', 'TO': 'Piemonte',
                'BA': 'Puglia', 'BO': 'Emilia-Romagna', 'FI': 'Toscana',
                'VE': 'Veneto', 'GE': 'Liguria', 'CA': 'Sardegna', 'PA': 'Sicilia',
                # Aggiungi altre province-regioni secondo necessità
            }
            
            def get_regione(provincia):
                if pd.isna(provincia):
                    return None
                # Estrai le prime due lettere della provincia (sigla)
                sigla = provincia[:2].upper()
                return regioni_map.get(sigla, None)
            
            df_filtrato['regione'] = df_filtrato['provincia'].apply(get_regione)
        
        df_filtrato = df_filtrato[df_filtrato["regione"].isin(regioni_sel)]

    if df_filtrato.empty:
        st.warning("⚠️ Nessun cliente trovato per i criteri selezionati.")
        return
    ...
    # Rimuovi clienti senza coordinate valide
    df_filtrato = df_filtrato.dropna(subset=["lat", "lon"])

    # Modifica: gestisci correttamente la selezione della città
    if citta_sel:
        # Filtra per città selezionata
        df_filtrato = df_filtrato[df_filtrato["citta"] == citta_sel]
        
        if df_filtrato.empty:
            st.warning(f"⚠️ Nessun cliente trovato nella città: {citta_sel}")
            return
        
        # Calcola il centro della mappa come media delle coordinate dei clienti nella città
        centro_mappa = [df_filtrato["lat"].mean(), df_filtrato["lon"].mean()]
        zoom_start = 12
        
        # Se è specificato un raggio, calcola le distanze e filtra
        if raggio_km is not None:
            # Calcola la distanza per ogni cliente
            distanze = []
            for _, row in df_filtrato.iterrows():
                distanza = calcola_distanza(centro_mappa[0], centro_mappa[1], row["lat"], row["lon"])
                distanze.append(distanza)

            df_filtrato["distanza_km"] = distanze
            df_filtrato = df_filtrato[df_filtrato["distanza_km"] <= raggio_km]
        
        # Se è specificato un numero di punti, filtra
        elif num_punti is not None:
            df_filtrato = df_filtrato.head(num_punti)
    else:
        # Mappa centrata sull'Italia
    ...
    # Controlla se ci sono clienti da mostrare
    if df_filtrato.empty:
        st.warning("⚠️ Nessun cliente trovato con i criteri selezionati.")
        return

    # CORREZIONE: Prima raggruppa per codice, poi filtra per numero di punti
    # Raggruppa i clienti per codice se presente
    # Creiamo un dizionario per raggruppare i clienti
    clienti_dict = {}
    
    for _, row in df_filtrato.iterrows():
        # Se il codice è presente, valido e non è 'nd', usiamolo come chiave
        if pd.notna(row['codice']) and row['codice'] != '' and row['codice'].lower() != 'nd':
            codice = row['codice']
            if codice not in clienti_dict:
                clienti_dict[codice] = {
                    'azienda': row['azienda'],
                    'citta': row['citta'],
                    ...
                    'matricole': [],  # Lista per le matricole esterne
                    'matint': [],     # Lista per le matricole interne
                    'num_attrezzature': 0  # Contatore del numero di attrezzature
                }
            
            # Incrementa il contatore di attrezzature
            clienti_dict[codice]['num_attrezzature'] += 1
            
            # Aggiungi la matricola se presente
            if pd.notna(row['matricola']) and row['matricola'] != '':
                clienti_dict[codice]['matricole'].append(row['matricola'])
            
            # Aggiungi la matricola interna se presente
            if pd.notna(row['matint']) and row['matint'] != '':
                clienti_dict[codice]['matint'].append(row['matint'])
        else:
            # Se non c'è codice, usa l'ID come chiave
            id_cliente = row['id']
            clienti_dict[id_cliente] = {
                'azienda': row['azienda'],
                'citta': row['citta'],
                'indirizzo': row['indirizzo'],
                'provincia': row['provincia'],
                ...
                'matricole': [],  # Lista per le matricole esterne
                'matint': [],     # Lista per le matricole interne
                'num_attrezzature': 1  # Contatore del numero di attrezzature
            }
            
            # Aggiungi la matricola se presente
            if pd.notna(row['matricola']) and row['matricola'] != '':
                clienti_dict[id_cliente]['matricole'].append(row['matricola'])
            
            # Aggiungi la matricola interna se presente
            if pd.notna(row['matint']) and row['matint'] != '':
                clienti_dict[id_cliente]['matint'].append(row['matint'])

    # Converti il dizionario in un DataFrame per il filtraggio
    df_raggruppati = pd.DataFrame.from_dict(clienti_dict, orient='index')
    
    # CORREZIONE: Filtra per numero di punti dopo il raggruppamento
    if num_punti is not None and citta_sel is None:
        df_raggruppati = df_raggruppati.head(num_punti)

    # Crea la mappa
    mappa = folium.Map(location=centro_mappa, zoom_start=zoom_start)

    # Aggiungi i marker dei clienti raggruppati
    for chiave, dati in df_raggruppati.iterrows():
        # Prepara le matricole e le matricole interne per il popup
        matricole_text = ""
        matint_text = ""
        
        # Aggiungi le matricole se presenti
        if dati['matricole']:
            matricole_text = "<br>".join([f"💬{m}" for m in dati['matricole']])
        
        # Aggiungi le matricole interne se presenti
        if dati['matint']:
            if matricole_text:  # Se ci sono già matricole, aggiungi una separazione
                matricole_text += "<br>"
            matricole_text += "<br>".join([f"💳{m}" for m in dati['matint']])
        
        # Se non ci sono né matricole né matricole interne
        if not matricole_text:
            matricole_text = "Nessuna attrezzatura registrata"
        
        # Prepara il codice punto vendita
        if isinstance(chiave, str) and chiave != str(dati.get('id', '')):
            codice_text = f"🏠 Punto vendita: {chiave}<br>"
        else:
            codice_text = ""
        
        # Prepara la proprietà
        proprieta_text = f"👤 Proprietà: {dati['proprieta']}<br>"
        
        # Usa il conteggio corretto delle attrezzature
        num_attrezzature = dati['num_attrezzature']
        ...
        🏛 {dati['azienda']}<br>
        <hr style="margin: 2px 0;">
        📍 {dati['indirizzo']} ({dati['provincia']})<br>
        <hr style="margin: 2px 0;">
        🖥 Attrezzature presso il PV : {num_attrezzature} <br>
        <hr style="margin: 2px 0;">
        Matricole <br>
        {matricole_text} {matint_text} 
        
        """
        
        # Crea il tooltip con il conteggio corretto delle attrezzature
        tooltip = f"{dati['azienda']} ({dati['citta']}, {dati['provincia']}) - Nr. attrezzature: {num_attrezzature}"
...
# --- Pagina Streamlit ---
def pagina_mappa_clienti():
    st.header("🏠 Mappa Clienti")

    # Verifica lo stato delle coordinate
    with st.expander("📊 Stato delle coordinate"):
        verifica_coordinate_clienti()

    # Aggiungi un'opzione per aggiornare tutte le coordinate
    with st.expander("🛠️ Opzioni avanzate"):
        st.write("Aggiorna le coordinate di tutti i clienti senza coordinate (potrebbe richiedere tempo)")
        if st.button("🔄 Aggiorna coordinate di tutti i clienti"):
            with st.spinner("Aggiornamento in corso..."):
                risultati = aggiorna_coordinate_clienti()
                st.success("Aggiornamento completato!")
                st.subheader("Risultati dell'aggiornamento:")
                for risultato in risultati:
                    st.write(risultato)
    ...
    conn = sqlite3.connect(DB_PATH)
    regioni = pd.read_sql_query("SELECT DISTINCT regione FROM clienti WHERE regione IS NOT NULL", conn)
    citta_disponibili = pd.read_sql_query("SELECT DISTINCT citta FROM clienti WHERE citta IS NOT NULL", conn)
    proprieta_disponibili = pd.read_sql_query("SELECT DISTINCT proprieta FROM clienti WHERE proprieta IS NOT NULL", conn)
    conn.close()
    ...
    lista_proprieta = ["Tutti"] + sorted(proprieta_disponibili["proprieta"].dropna().tolist())
    proprieta_sel = st.selectbox("👤 Seleziona proprietà:", lista_proprieta, index=0)

    # Input per la città con suggerimenti
    st.subheader("🔍 Cerca una città")
    
    use_suggested = st.checkbox("Usa città dove sono presenti i clienti", value=True)
    ...
    # Checkbox per mostrare i tecnici vicini
    mostra_tecnici_vicini = st.checkbox("Mostra tecnici vicini alla città selezionata", value=False)
    
    if mostra_tecnici_vicini and citta_sel:
        # Impostazioni per la ricerca dei tecnici
        st.subheader("🔧 Impostazioni ricerca tecnici")
        col1, col2 = st.columns(2)
        
        with col1:
            raggio_tecnici = st.number_input("Raggio ricerca tecnici (km):", min_value=1, value=50)
        with col2:
            num_tecnici = st.number_input("Numero di tecnici da mostrare:", min_value=1, value=5)
        
        # Mostra i tecnici vicini
        st.subheader("👷 Tecnici vicini a " + citta_sel)
        mostra_tecnici_vicini_citta(citta_sel, raggio_tecnici, num_tecnici)
    
    # Mostra la mappa dei clienti
    st.subheader("🏠 Mappa Clienti")
    
    # Modifica: gestisci correttamente la selezione della città
    if citta_sel:
        # Se stiamo usando la selectbox, non geocodificare
        if use_suggested:
            mostra_mappa_clienti(regioni_sel, citta_sel, raggio_km, num_punti, proprieta_sel)
        else:
            # Se stiamo inserendo una città manualmente, geocodifichiamola
            lat_citta, lon_citta = geocodifica(citta_sel)
            if lat_citta is None or lon_citta is None:
                st.error(f"⚠️ Impossibile trovare le coordinate per la città: {citta_sel}")
                st.info("💡 Suggerimento: Prova a specificare anche la provincia (es: 'Roma, RM')")
        ...
        mostra_mappa_clienti(regioni_sel, None, None, None, proprieta_sel)

# Funzione per mostrare i tecnici vicini a una città
def mostra_tecnici_vicini_citta(citta, raggio_km=50, num_tecnici=5):
    # Geocodifica la città
    lat_citta, lon_citta = geocodifica(citta)
    if lat_citta is None or lon_citta is None:
        st.error(f"⚠️ Impossibile trovare le coordinate per la città: {citta}")
        st.info("💡 Suggerimento: Prova a specificare anche la provincia (es: 'Roma, RM')")
        return
    
    conn = sqlite3.connect(DB_PATH)
    df_tecnici = pd.read_sql_query(
        "SELECT id, esperienza, nome, citta, provincia, regione, telefono, cellulare, referente, email, lat, lon FROM tecnici", conn
    )
    conn.close()
    ...
    # Pulisci i dati delle coordinate
    df_tecnici['lat'] = pd.to_numeric(df_tecnici['lat'], errors='coerce')
    df_tecnici['lon'] = pd.to_numeric(df_tecnici['lon'], errors='coerce')
    
    # Rimuovi tecnici senza coordinate valide
    df_tecnici = df_tecnici.dropna(subset=["lat", "lon"])
    
    if df_tecnici.empty:
        st.warning("⚠️ Nessun tecnico con coordinate valide.")
        return
    ...
    df_tecnici["distanza_km"] = distanze
    
    # Rimuovi tecnici senza distanza valida
    df_tecnici = df_tecnici.dropna(subset=["distanza_km"])
    
    # Filtra per raggio
    df_tecnici = df_tecnici[df_tecnici["distanza_km"] <= raggio_km]
    
    # Ordina per distanza
    df_tecnici = df_tecnici.sort_values(by="distanza_km")
    
    # Filtra per numero di punti
    df_tecnici = df_tecnici.head(num_tecnici)
    
    if df_tecnici.empty:
        st.warning(f"⚠️ Nessun tecnico trovato entro {raggio_km} km da {citta}")
        return
    
    # Crea la mappa
    mappa = folium.Map(location=[lat_citta, lon_citta], zoom_start=10)
    
    # Aggiungi un marker per la città selezionata
    folium.Marker(
        location=[lat_citta, lon_citta],
        popup=f"📍 {citta}",
        icon=folium.Icon(color="red", icon="home", prefix="fa")
   ...
   # Mappatura esperienza -> (icona + descrizione)
    ...
    for _, row in df_tecnici.iterrows():
        ...
        esp_info = esperienza_info.get(row['esperienza'], {})
        icona = esp_info.get('icona', '')
        descrizione = esp_info.get('descrizione', '')
...
# ROUTER PAGINE 
# ===========================================
if menu == menu_options[0]:  # 🏠 Gestione PV
    pagina_clienti()
elif menu == menu_options[1]:  # 🗺️ Mappa PV
    pagina_mappa_clienti()
elif menu == menu_options[2]:  # 👷 Gestione Tecnici
    pagina_tecnici()
elif menu == menu_options[3]:  # 🗺️ Mappa Tecnici
    pagina_mappa_tecnici()
elif menu == menu_options[4]:  # 🎫 Ticket di Assistenza
    pagina_ticket()
elif menu == menu_options[5]:  # 🖨️ Stampa Ticket
    pagina_stampa_ticket()
elif menu == menu_options[6]:  # 📊 Analisi Ticket
    pagina_analisi_ticket()
elif menu == menu_options[7]:  # 🛠️ Tools
    pagina_tools()
elif menu == menu_options[8]:  # ⬆️ Upload Clienti
    pagina_upload_clienti()
elif menu == menu_options[9]:  # ⬆️ Upload Tecnici
